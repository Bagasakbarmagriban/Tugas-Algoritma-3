def read_data(file_name):
    print(f"Mencoba membuka file: {file_name}")
    data = []
    try:
...
def calculate_statistics(data):
    ...
    peserta_terendah = min(peserta_lulus, key=lambda x: x[1], default=(None, None))  
    return total_peserta, rata_rata, peserta_tertinggi, peserta_terendah, peserta_lulus

def write_results(file_name, total_peserta, rata_rata, peserta_tertinggi, peserta_terendah, peserta_lulus, data_file, result_file):
...
data_file = input("Ketik nama file data: ").strip()  
result_file = input("Ketik nama file hasil: ").strip()  
...
if data:
    ...
    write_results(result_file, total_peserta, rata_rata, peserta_tertinggi, peserta_terendah, peserta_lulus, data_file, result_file)
